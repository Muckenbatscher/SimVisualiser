...
{
    internal class BindTCGameEventService : GameSenseServiceBase, IBindGameEventService
    {
        private readonly BindGameEventEndpoint _endPoint;

        private readonly ISampleMessageGeneration<BindGameEventMessage> _tcMessageGeneration;

        public BindTCGameEventService(TargetAddress baseAddress) : base(baseAddress)
        {
            _endPoint = new BindGameEventEndpoint(baseAddress.GetURL());
            _tcMessageGeneration = new TCEventBindingMessageService();
        }

        public void BindEvent()
        {
            var message = _tcMessageGeneration.GetFilledMessage();
