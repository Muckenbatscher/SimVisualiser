...
{
    internal class TCEventBindingMessageService : ISampleMessageGeneration<BindGameEventMessage>
    {
        private readonly BindingHandlerGenerationService _bindingHandlerGeneration;

        public TCEventBindingMessageService()
        {
        ...
        public BindGameEventMessage GetFilledMessage()
        {
            var message = new BindGameEventMessage()
            {
                ...
                EventName = GameEventMetadata.TC_ACTIVE_EVENT_NAME,
                MinValue = 0,
                MaxValue = 1
            };
